...
def test_lnk():
    f = File(contents=open(b"tests/files/lnk_1.lnk", "rb").read())
...
def test_ole():
    msg = open(b"tests/files/msg_invoice.msg", "rb").read()
    bup = open(b"tests/files/bup_test.bup", "rb").read()
...
@pytest.mark.xfail
def test_bup():
    mso = unpack(b"tests/files/msg_invoice.msg").read(b"oledata.mso")
